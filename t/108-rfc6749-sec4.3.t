# Test RFC6749 section 4.3 (https://datatracker.ietf.org/doc/html/rfc6749#section-4.3)
...
my $mock-server = Cro::HTTP::Server.new:
        ...
        application => route {
            ...
            post -> 'token' {
                request-body -> (:$grant_type, :$username, :$password, :$scope, :$client_id,
                                 :$client_secret) {
                    subtest "Request sent to authorization server", {
                        plan 6;
                        is $grant_type, 'password', "grant_type is password";
                        is $username, $expected-username, "good username receive";
                        is $password, $expected-password, "good password receive";
...
$mock-server.start;
my $authorization = $oauth.authorization: :username($expected-username), :password($expected-password), :scope(A,);
$mock-server.stop;
