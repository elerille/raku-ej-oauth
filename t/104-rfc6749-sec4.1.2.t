# Test RFC6749 section 4.1.2 (https://datatracker.ietf.org/doc/html/rfc6749#section-4.1.2)
...
subtest "without state", {
    plan 2;
    dies-ok { $oauth.authorization-response(code => $expected-code) }, "Receive without state is an error";
...
subtest "with false state", {
    ...
    if $state eq $error-state {
        ...
        dies-ok { $oauth.authorization-response(:state($error-state), code => $expected-code) },
                "Receive with false state is an error";
    }
    is $authorization.status, Planned, "Authorization isn't now valided";
}
